...
@section('content')
    ...
    <?php
                ...
                <a href="{{URL::to('/attendance_management/student_attendance_view')}}" class="shortcut"><i
                            ...
                            class="shortcut-label">Attendance Information</span> </a>


                <a href="{{URL::to('/assignment_management/student_assignment')}}" class="shortcut"><i
                            class="shortcut-icon icon-file-text-alt icon-2x"></i><span class="shortcut-label">Assignment Information</span>
                </a>
                <?php
                ...
                //$classsectionid = StudentResult::where('S_ID','=',$st_sess)->groupby('S_ID')->first();
                $classsectionid = TStudentResult::where('st_id','=',$st_sess)->groupby('st_id')->first();

                if(count($classsectionid)){

                    $class_name = $classsectionid->class;
                    $section = $classsectionid->Section;
                    $csid = $classsectionid->idclasssection;}
                else
                {
                    $class_name = "";
                    $section = "";
                ...
                $termF = PublishResult::where('published','=','Y')->where('class','=',$class_name)->where('section','=',$section)->where('term','=','Final')->first();

                $y= date('Y'); $yn= date('Y')+1;
                $current_year =$y."-".$yn;

                $term_countF= count($termF);

                $current_term = "Half Yearly";
                if($term_countF)  $current_term = "Final";

                $pb= "N";
                $pb = PublishResult::where('class','=',$class_name)->where('section','=',$section)->where('term','=',$current_term)->pluck('published');

                ?>

                @if($pb == 'N')

                    <a href="{{ URL::to('/report12/'.$st_sess.'/'.$current_year.'/'.$current_term.'/'.$csid)}}" class="shortcut"><i class="shortcut-icon icon-user"></i><span class="shortcut-label">View Report Card</span> </a>
                @else

                    <a href="{{ URL::to('/result_management/student_report_card')}}" class="shortcut"><i class="shortcut-icon icon-user"></i><span class="shortcut-label">Student Report</span> </a>

                @endif

                <a href="{{URL::to('/fees_management/student_fees_payslip_monthwise_view')}}" class="shortcut"><i
                            class="shortcut-icon icon-dollar"></i> <span class="shortcut-label">Fees&nbsp;&nbsp;&nbsp;&nbsp; Information</span>
                </a>
                                                    ...
                                                    @if ($date[$count_day1]=="yes")
                                                        @if($types[$count_day1]!=null)
                                                            ...
                                                            @if($type=="event")
                                                                ...
                                                                <td   bgcolor="#66FFFF" class="modal-td"onclick="myFunctionEvent('<?php echo $id; ?>','<?php echo addslashes(htmlspecialchars($eventdesc));?>','<?php if(isset($startDate)) echo $startDate;?>','<?php if(isset($endDate)) echo $endDate;?>','<?php echo $startTime;?>','<?php echo $endTime;?>','<?php echo $eventPlace;?>','<?php echo $eventName;?>' )">

                                                            @elseif($type=="annual_calender")
                                                                <td  bgcolor="#66FFFF" class="modal-td"onclick="myFunctionAnualCalender('<?php echo $id; ?>','<?php echo addslashes(htmlspecialchars($eventdesc)); ?>','<?php if(isset($startDate)) echo $startDate; ?>','<?php if(isset($endDate)) echo $endDate; ?>','<?php echo $nameOfHolyday; ?>','<?php echo $typeOfHolyday; ?>' )">
    ...
    <div class="span6">
        ...
        <div class="widget widget-nopad">
            ...
            <div class="widget-content">
                <ul class="news-items">
                    <?php
 $notice = Notice::where('notice_type','Students')->orwhere('notice_type','Public')->OrderBy('idnotice','desc')->take(5)->get();?>
                    ...
                    <br/>
                    <center><a href="{{URL::to('/notice_management/show_notice_all')}}"><button class="btn btn-success">Show All </button></a></center>
                    ...
                    <div class="widget big-stats-container">
                        <div class="widget-content">
                            ...
                            <div id="big_stats" class="cf">
                                ...
                                <div class="stat">Active Students (web) <span class="value">2236</span> </div>
                                <!-- .stat -->
