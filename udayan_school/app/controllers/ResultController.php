...
class ResultController extends BaseController {
    ...
    //debug
    function is_pass($mark_conf, $cls_val, $mark, $type, $flag = 0)
    {
        ...
        if($flag == 1) {
            if ($cls_val > 8 && ($type == "MT" || $type == "HT")) {
                $range = $mark_config->total_marks;   
            }
        }
...
$ct_total = $ct_conf[0]->weighted_marks + $ct_conf[1]->weighted_marks;
 ...
 if($ct_conf[0]->ct_type == "3") { $ct_total = $ct_conf[1]->weighted_marks; $ct2 = 0;} }
        ...
        foreach ($student_info as $student_info) {
            ...
            foreach($mark_config as $config) {
                if ($config->configuration_type == "RT") { // Regular assessment
                    ...
                    $st_rslt_rt = ResultRegularAssessment::where('studentinfo_idstudentinfo', $student_info->student_idstudentinfo)->where('class', $class_name)->where('section', $class_sec)->where('idclasssection', $class_section_id)->where('idsubject', $subject_id)->where('term', $term)->where('academic_year', $year)->first();
                    ...
                    if(!(($class_num == 5 || $class_num == 8 || $class_num == 10) && strtolower($term) == 'final'))
                    {
                        ...
                        $st_rslt_ct = ResultClassTest::where('studentinfo_idstudentinfo', $student_info->student_idstudentinfo)
                        ->where('class', $class_name)
                        ->where('section', $class_sec)
                        ->where('idclasssection', $class_section_id)
                        //->where('courseteacher_idcourseteacher', $course_teacher_id)
   ...
   /*                         
                            //check for pass mark for class 9 and above and also checking class test pass mark
                            ...
                            if($config->ct_type == 2) {
                                ...
                                if($class_num < 9) { //ignore ct pass check if class is above 8

                                    // check for ct_cq, ct_mcq pass except for class 11
...
*/
                    ...
                    $st_rslt_mcq = ResultMCQTest::where('studentinfo_idstudentinfo', $student_info->student_idstudentinfo)->where('class', $class_name)->where('section', $class_sec)->where('idclasssection', $class_section_id)
// ->where('courseteacher_idcourseteacher', $course_teacher_id)
->where('idsubject', $subject_id)->where('term', $term)->where('academic_year', $year)->first();
                    // get mcq mark
                    ...
                    if ($class_num > 8) {
                        ...
                        if (!(($class_num == 10) && (strtolower($term) == 'final'))) {
                            ...
                            //$tmp['ct_mcq'] = 0;
                            if($class_num > 8) { //total mcq mark check
                                if($pass_flag) {
                                    $pass_flag = ResultController::is_pass($config, $class_num, $mcq_mark, "MT", 1);
                                }
                            }
                        }
                    }
                    ...
                    // get hall test mark
                    $st_rslt_ht = ResultHallTest::where('studentinfo_idstudentinfo', $student_info->student_idstudentinfo)
                        ->where('class', $class_name)
                        ->where('section', $class_sec)
                        ->where('idclasssection', $class_section_id)
                       // ->where('courseteacher_idcourseteacher', $course_teacher_id)
                    ...
                    if($class_num > 8) {
                        ...
                        if($config->ct_type=='2' || $config->ct_type==2){
                            if($pass_flag) {
                                 $pass_flag = ResultController::is_pass($config, $class_num, $ht_mark, "HT"); 
                            }
                        ...
                        if (!(($class_num == 10) && (strtolower($term) == 'final'))) {
                            ...
                            if($class_num > 8) { 
                                ...
                                if(($config->ct_type!='2' || $config->ct_type!=2)){
                                    if($pass_flag) {
                                        $pass_flag = ResultController::is_pass($config, $class_num, $ht_mark, "HT", 1);
                                    }
                                }
                            }
...
/*
           ...
           if($student_info->student_idstudentinfo=='2015903223' || $student_info->student_idstudentinfo=="2015903223")
{


                echo " ". $student_info->student_idstudentinfo;

                 return  "error" . $ht_mark;  
}   
...
*/
                    ...
                    $st_rslt_lt = ResultLabTest::where('studentinfo_idstudentinfo', $student_info->student_idstudentinfo)
                                      ->where('class', $class_name)->where('section', $class_sec)
                                      ->where('idclasssection', $class_section_id)
                                      ->where('courseteacher_idcourseteacher', $course_teacher_id)
                                      ->where('idsubject', $subject_id)->where('term', $term)
                                      ->where('academic_year', $year)
                                      ->first();
 ...
 $ft = $result['f_ra']+$result['f_ct']+$result['f_mcq']+$result['f_ht']+$result['f_lab'];
                    } else {
 ...
 //      if($student_info->student_idstudentinfo=='2015905415')            
           ...
           if($student_info->student_idstudentinfo=='2015903223' || $student_info->student_idstudentinfo=="2015903223")
{
                ...
                echo " ". $student_info->student_idstudentinfo;
                 ...
                 return  "error" . $ht_mark . "--" .$result['gt_total'];  
} 
...
*/
            ...
            if($class_num > 8) {
               ...
               //cq
                ...
                $conv_res['cq_conv'] = $conv_res['cq_ct'] + $conv_res['cq_total'];
                ...
                */
                ...
                $conv_res['mcq_conv'] = $conv_res['mcq_ct'] + $conv_res['mcq_total'];
                ...
                if(!count($conv_marks)) {
                  ...
                  ConvertedMarks::where('st_id', $student_info->student_idstudentinfo)->where('subjectid', $subject_id)->where('class_id', $class_info->class_id)->where('term', $term)->where('year', $year)->update($conv_res);
                      ...
                      //  echo count($conv_marks )."<br/>".$student_info->student_idstudentinfo . "<br/>". $pass_flag ."<br/>";
                        //  return "<br/>check13";
                }
            }
        } // end of foreach student
    ...
    public function resultindividual()
    {
        ...
        $subject = SubjectToClass::where('class','=',$class->class)->join('subject', 'subject.idsubject', '=', 'subjecttoclass.idsubject')->get();
