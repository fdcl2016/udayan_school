...
class ResultController extends BaseController {
    ...
    function is_pass($mark_conf, $cls_val, $mark)
    {
        //  $mark_conf = MarksConfiguration::where('configuration_name','=',$mark_conf)->where('configuration_type','=',$type)->first();
        if(!count($mark_conf) || !$mark_conf ) return 1;
        if (isset($mark_conf->converted_marks)) $range = $mark_conf->converted_marks; else return 1;

        if($mark_conf->configuration_type == "LT" || $cls_val == 9 || $cls_val == 10)
            $percent = 40;
        else
            $percent = 33;
    ...
    public function prepare_marksheet($term, $year, $class_name, $class_sec, $course_teacher_id) {
        ...
        */
        ...
        foreach ($student_info as $student_info) {
            ...
            $stdnt_rslt = TStudentResult::where('st_id', $student_info->student_idstudentinfo)->where('idclasssection', $class_section_id)->where('subjectid', $subject_id)->where('idcourseteacher', $course_teacher_id)->where('academic_year', $year)->first();
            ...
            foreach($mark_config as $config) {
                if ($config->configuration_type == "RT") { // Regular assessment
                    ...
                    (count($st_rslt_rt->classwork) ? $cw_mark = $st_rslt_rt->classwork : $cw_mark = 0);
                    (count($st_rslt_rt->homework) ? $hw_mark = $st_rslt_rt->homework : $hw_mark = 0);
                    // total rt mark
                    ...
                    if(strtolower($term) == 'final') {
                        ...
                        if($class_num < 11)
                            $result['gt_ra'] = $rt_mark + $stdnt_rslt->h_ra;
                    } else {
                        $result['h_ra'] = $rt_mark;
                    }
                    ...
                    if(!(($class_num == 5 || $class_num == 8 || $class_num == 10) && strtolower($term) == 'final'))
                    {
                        ...
                        if ($class_num <= 8) {
                            // result details
                            (count($st_rslt_ct->ct1) ? $ct1 = $st_rslt_ct->ct1 : $ct1 = 0);

                            $ct_mark = $ct1;
                        } else {
                            // get result details
                            (count($st_rslt_ct->ct1) ? $ct_cq = $st_rslt_ct->ct1 : $ct_cq = 0);
                            (count($st_rslt_ct->ct2) ? $ct_mcq = $st_rslt_ct->ct2 : $ct_mcq = 0);
                            // total CT marks
                            ...
                            */
                            ...
                            if($config->ct_type == 2) {
                                $pass_flag = (ResultController::is_pass($config, $class_num, $ct_cq) ? true : false);
                            } else if ($config->ct_type == 4) {
                                $pass_flag = (ResultController::is_pass($config, $class_num, $ct_mcq) ? true : false);
                            } else {
                                if(!(ResultController::is_pass($config, $class_num, $ct_cq) && ResultController::is_pass($config, $class_num, $ct_mcq)))
                                    $pass_flag = false;
                            }
                        }
                        //keep data to update to db
                    ...
                    // get mcq mark
                    (count($st_rslt_mcq->mcq_marks) ? $mcq_mark = $st_rslt_mcq->mcq_marks : $mcq_mark = 0);

                    if ($class_num > 8) {
                        if($pass_flag) {
                            $pass_flag = ResultController::is_pass($config, $class_num, $mcq_mark);
                        }
                    ...
                    // get hall test mark
                    (count($st_rslt_ht->hall_test_marks) ? $ht_mark = $st_rslt_ht->hall_test_marks : $ht_mark = 0);

                    if($class_num > 8) {
                        if($pass_flag) {
                            $pass_flag = ResultController::is_pass($config, $class_num, $ht_mark);
                        }
                        if (!(($class_num == 10) && (strtolower($term) == 'final'))) {
                    ...
                    //get lab test mark
                    (count($st_rslt_lt->viva_marks) ? $lt_mark = $st_rslt_lt->viva_marks : $lt_mark = 0);
                    // keep data to update to db
                    if(strtolower($term) == 'final') {
            ...
            if($class_num < 11) {
                if(strtolower($term) == 'final') {
                    ...
                    $result['gt_total'] = ($stdnt_rslt->h_total) + $result['f_total'];
                ...
                TStudentResult::where('st_id', $student_info->student_idstudentinfo)->where('idclasssection', $class_section_id)->where('subjectid', $subject_id)->where('idcourseteacher', $course_teacher_id)->where('academic_year', $year)->update($result);
            }
                ...
                */
                ...
                if($class_num < 11) {
                    ...
                    if(strtolower($term) == 'final')
                    {
                        ...
                        $conv_res['grade'] = $conv_grade['point'];
                        $conv_res['point'] = $conv_grade['letter'];
                    } else {
