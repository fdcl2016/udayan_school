...
<body>';
        ...
        while($row = $result->fetch_assoc()) {
            ...
            $sql2="SELECT * FROM t_st_result INNER JOIN subject ON t_st_result.subjectid = subject.idsubject WHERE t_st_result.subjectid!= $r AND  t_st_result.st_id = 2016911310 Order by subject.priority";

            $result2= $conn->query($sql2);
            $row2 = $result1->fetch_assoc();

            $sql3="SELECT rank,grade,counter_position,total_mark,comment,cgpa FROM student_rank WHERE student_id = 2016911310";
...
<div style="width:130px;float:left;margin-left:12px;margin-top: 2px">';
  $im = StudentInfo::where('idstudentinfo','=',2016911310)->where('image','=','')->get();
    ...
    ';
        ...
        <table  style="border-collapse: collapse;letter-spacing: 1px;width: 100%;font:arial" border="1" >
          ...
          //  }
            ...
            $sql5="SELECT * FROM t_st_result where subjectid = $r AND st_id = 2016911310 ";

            $result5 = $conn->query($sql5);
            $row5 = $result5->fetch_assoc();


            $total_marks1 = $row5['total'];

            $sub = SubjectToClass::where('class','=',$row5['class'])->where('idsubject','=',$row5['subjectid'])->pluck('markconfiguration_name');
            $cls_val = Addclass::where('class_name','=',$row2['class'])->where('section','=',$row2['section'])->pluck('value');


            if ($row5['f_ct'] != null) $ct_marks = $row5['f_ct'];else $ct_marks=0 ;
            if ($row5['f_ra'] != null) $ra_marks = $row5['f_ra'];else $ra_marks=0 ;
            if ($row5['f_lab'] != null) $lab_marks = $row5['f_lab'];else $lab_marks=0 ;
            if ($row5['f_mcq'] != null) $mcq_marks = $row5['f_mcq'];else $mcq_marks=0 ;
            if ($row5['f_ht'] != null) $ht_marks = $row5['f_ht'];else $ht_marks=0 ;
            if ($row5['f_total'] != null) $total = $row5['f_total'];else $total=0 ;


            $point = $row2['f_gp'];
            $grade = $row2['f_grade'];

            $grade = GradingTable::where('total', '=', $total_marks1)->where('highest_range', '>=', $total)->where('lowest_range', '<=', $total)->first();
            //echo $result->sutdent_name." : ".$grade."--".$total_get_marks."------->";
            $cls = $row5['class'];
            $gp = $grade->grade;
            $gpa = $grade->gpa;
...
$student_rank = RankModel::where('stid','=','2016911310')->pluck('rank');
            ...
            $html.='<td rowspan="2" style="text-align: center">'.$student_rank.'</td><td style="text-align: center">Working Days</td><td style="text-align: center">N/A</td>
