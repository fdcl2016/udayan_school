...
//////////////////////////////////Ajax Here
Route::get('/ajaxyearsection','AjaxController@ajaxyearsection');
...
/* Custom report */
Route::get('result_management/custom_report','ReportsController@getCustomReport')->before('auth');
Route::post('result_management/custom_report','ReportsController@postCustomReport');

Route::get('report_management/custom_report_pdf','PdfControllerReport@section_report_pdf')->before('auth');

/* Admin Custom report */
Route::get('result_management/admin_custom_report','ReportsController@getAdminCustomReport')->before('auth');
Route::post('result_management/admin_custom_report','ReportsController@postAdminCustomReport');

Route::get('report_management/admin_custom_report_pdf','PdfControllerReport@admin_report_pdf')->before('auth');

/* end of custorm report*/


/****************************************** SIAM - 01/12/2016 *************************************************/


Route::get('result_management/student_report_card', 'ReportsController@getStudentReportCard')->before('auth');

Route::post('result_management/student_report_card', 'ReportsController@postStudentReportCard');
...
//$port = "8080";
   ...
   $ch = curl_init("http://$host/sendsms.jsp?");
    ...
    curl_setopt($ch, CURLOPT_POST, True);
    //curl_setopt($ch, CURLOPT_POSTFIELDS,"username=$username&password=$password&type=0&dlr=1&destination=$mobile&source=$sender&message=$message_final");
    curl_setopt($ch, CURLOPT_POSTFIELDS,"user=$username&password=$password&mobiles=$mobile&sms=$message_final&clientsmsid=10001&senderid=$sender&scheduletime=0");
    curl_setopt($ch, CURLOPT_TIMEOUT,60);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, True);
...
if($classsec->value > 10) {
            $c_teacher = ResultClassTest::where('result_class_test.idsubject',$idsubject)
                ->where('result_class_test.idclasssection',$classname)
                ->where('academic_year', $year)
                ->where('term', $term)
                ->leftjoin('courseteacher', 'courseteacher.idcourseteacher', '=', 'result_class_test.courseteacher_idcourseteacher')
                ->pluck('idteacherinfo');
...
$result=TStudentResult::distinct()->where('idclasssection','=',$classname)
            ->where('subject','=',$subject)
            ->where('academic_year','=',$year)->leftjoin('student_to_section_update', 'student_to_section_update.student_idstudentinfo', '=', 't_st_result.st_id')
            ->orderby('student_to_section_update.st_roll', 'ASC')->groupBy('student_to_section_update.st_roll')->get();
...
Route::post('publish',function(){
    ...
    PublishResult::where('term','=',$t)->where('year','=',$y)->where('approved','=','N')->update($data);

   // $message = "Result Published successfully";

    //return "success";
    
  //  return View::make('result_management.publish_result')->with('msg',$message);
...
Route::post('list_st',function(){
    ...
    $classname = Input::get('class');
    $sectionname = Input::get('sec');


    // return $classname;

       // $st = ClassWiseStd::where('std_class','=',$classname )->where('std_section','=',$sectionname)->get();
        $st = StudentToSectionUpdate::where('section','=',$sectionname)->orderBy('st_roll')->get();

 $clas = StudentToSectionUpdate::where('section','=',$sectionname)->pluck('class');

    $st_count = StudentToSectionUpdate::where('section','=',$sectionname)->get();

    $male = ClassWiseStd::where('std_gender','=','Male')->where('std_section','=',$sectionname)->get();
...
Route::post('pdf',function()
{
 ...
 $class_section= Addclass::where('class_name','=',$classname)->where('section','=',$sectionname)->pluck('class_id');
    ...
    $results=ConvertedMarks::where('class_id','=',$class_section)
            ->where('converted_marks.year','=','2016-2017')
            ->where('subjectid','=',$idsubject)
...
Route::post('view_tabulationsheet', function ()
{
    ...
    $year = Input::get('year');
    $term = Input::get('term');
    if(!(isset($term) && strtolower($term) == 'final')) {
    //  if term is final then show else redirect
    $msg = "<div class='alert alert-danger'><strong><h3 style='color:black'         align='center'>Tabulation sheet can be viewed for the final term only</h3></strong></div>";
    return Redirect::to('/view_tabulationsheet')->with('msg', $msg);
} else {
...
Route::get('view_tabulationsheet', function () {
    ...
    */
    if(isset($class_section)) {
        ...
        $class_info = Addclass::where('class_id', $class_section)->first();
        $classname = $class_info->class_name;
        $sectionname = $class_info->section;
        $val = $class_info->value;
    } else {
        $classname = null;
        $sectionname = null;
        $val = null;
...
if(Input::get('classname')=='NINE'){

return View::make('result_management.teachertabulation_nine')->with('class',Input::get('classname'))
...
{
    $msg = $msg = Session::get('msg');

    return View::make('result_management.st_report_card2')
            ->with('msg', $msg);
...
{
    // $class_section = explode(" ", Input::get('class_section'));
    // $class = $class_section[0];
    // $section = $class_section[1];

    $class = 'TWO';
    $section = Input::get('class_section');
    $term = Input::get('term');
    $year = Input::get('year');
    ...
    if(false) {
        // Result not publisehd; redirect with error msg
        $msg = "<div class='alert alert-danger'><strong><h3 style='color:black' align='center'>Result is not publisehd yet</h3></strong></div>";
    } else {
        // result published redirect to download

        if($class_val == 9) {

            return Redirect::to('reportcard_management/classNineReportCard/')
                        ->with('class', $class)
                        ->with('section', $section)
                        ->with('term', $term)
                        ->with('year', $year);
        } elseif($class_val > 9) {
            return Redirect::to('reportcard_management/getfile12/'.$class.'/'.$section.'/'.$term.'/'.$year);
        }else{


  return Redirect::to('reportcard_management/getfile/'.$class.'/'.$section.'/'.$term.'/'.$year);

        }
        
    }

    return Redirect::to('result_management/st_report_card2')
                ->with('msg', $msg);
});
...
Route::get('reportcard_management/getfile12/{class}/{sec}/{term}/{year}','PdfControllerCCopy@pdf')->before('auth');


Route::get('reportcard_management/classNineReportCard/','PdfControllerCCopyForNine@class_nine_report_card')->before('auth');



/*

Route::get('reportcard_management/getfile/{sec}/{term}/{year}','PdfControllerC@pdf')->before('auth');

Route::get('reportcard_management/getfile12/{sec}/{term}/{year}','PdfControllerCCopy@pdf')->before('auth');
...
Route::get('attendance_management/attendance_book',function()
{
    ...
    if ($classsection!=null) {
      ...
      $subid = CourseTeacher::where('idclasssection','=',$classsection)->where('idteacherinfo','=',Auth::user()->user_id)->pluck('idsubject');
        ...
        $attendance = AttendanceView::distinct()->where('Month','=',$month)->where('Class_Section_Id','=',$classsection)->where('idteacher','=',Auth::user()->user_id)->get();
...
Route::get('submit_marks',function()
{
    if(Auth::user()->type=="teacher") {
        ...
        $term = PublishResult::where('published','=','Y')->where('class','=',$class_name)->where('section','=',$section)->where('term','=','Final')->first();
        $y= date('Y'); $yn= date('Y')+1;
        $current_year =$y."-".$yn;
        $term_count= count($term);
        if($term_count>0) $current_term = "Half Yearly"; else $current_term = "Final";
...
$current_term = "Half Yearly";
        ...
        $plist = TabulationSheetEditable::distinct()->where('term','=',$current_term)->where('academic_year','=',$current_year)->where('class','=',$class_name)->where('section','=',$section)->groupBy('idsubject')->get();
